...
Phase 1 delivers the right-hand column (schema + stubs) and the left-hand middleware patch + env vars. No wiring between them happens yet — internalMutations are not called by anything until Phase 2's webhook handler exists. This is intentional (D-07): the phase's job is that everything _compiles and deploys_, not that it does anything.
...
_(These are pre-existing infra gaps surfaced by this phase, not necessarily Phase 1 deliverables — planner should decide whether to address them in Wave 0 of this phase or defer.)_
